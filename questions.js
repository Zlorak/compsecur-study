...
const QUESTIONS = [
  ...
  { cat: "Cryptography", q: "Which provides perfect forward secrecy?", o: ["Reusing the same RSA key for every session", "Ephemeral key exchange (e.g., ECDHE)", "ECB mode", "Storing keys in plaintext"], a: 1, e: "Ephemeral Diffie-Hellman (DHE/ECDHE) generates a fresh key per session, so compromise of a long-term key doesn't expose past traffic." },
  { cat: "Cryptography", q: "A company must encrypt a laptop's entire disk so data is safe if stolen. Best choice?", o: ["TLS", "Full-disk encryption (e.g., BitLocker), ideally with a TPM", "Hashing the files", "Steganography"], a: 1, e: "FDE protects data at rest. A TPM stores the key in hardware and ties it to the platform, preventing the disk being read in another machine." },
  { cat: "Cryptography", q: "What does a TPM provide?", o: ["A network firewall", "A hardware root of trust for key storage and platform integrity", "An antivirus engine", "A password manager in the cloud"], a: 1, e: "A Trusted Platform Module is a chip on the motherboard that securely stores keys and supports secure boot / measured boot. An HSM is the removable/enterprise equivalent." },
  ...
  { cat: "Threats & Attacks", q: "A script stored on a forum runs in other users' browsers when they view a post. This is:", o: ["Stored (persistent) XSS", "Reflected XSS", "SQL injection", "Directory traversal"], a: 0, e: "Malicious script saved on the server and served to victims = stored/persistent XSS. Reflected XSS bounces off a crafted link." },
  { cat: "Threats & Attacks", q: "An attacker intercepts and possibly alters traffic between two parties. This is a(n):", o: ["On-path (man-in-the-middle) attack", "Replay attack", "Privilege escalation", "Watering hole"], a: 0, e: "Sitting between endpoints is an on-path / MITM attack. Mitigate with mutual TLS, certificate validation, and HSTS." },
  ...
  { cat: "Threats & Attacks", q: "Registering 'gooogle.com' to catch mistyped traffic is:", o: ["Typosquatting (URL hijacking)", "Pharming", "Pretexting", "Shoulder surfing"], a: 0, e: "Exploiting typos in domain names is typosquatting/URL hijacking." },
  ...
  { cat: "Malware", q: "Which control most directly limits damage if ransomware encrypts a server?", o: ["Offline, tested backups (3-2-1)", "A stronger password policy", "Disabling USB ports", "More CPU cores"], a: 0, e: "Recoverable, isolated backups let you restore without paying. 3-2-1: 3 copies, 2 media, 1 offsite/offline." },
  ...
  { cat: "IAM", q: "A user logs in once and accesses multiple apps without re-authenticating. This is:", o: ["Single sign-on (SSO)", "Federation only", "MFA", "Just-in-time access"], a: 0, e: "SSO gives one authentication for many services. Federation extends trust across organizations (e.g., SAML/OIDC)." },
  ...
  { cat: "Network Security", q: "A segment that hosts public-facing servers, isolated from the internal LAN, is a:", o: ["Screened subnet (DMZ)", "VLAN 1", "Honeynet", "VPN concentrator"], a: 0, e: "A screened subnet (formerly DMZ) exposes services like web/mail while shielding the internal network." },
  ...
  { cat: "Network Security", q: "Which best protects data in transit for a remote worker connecting to the corporate network?", o: ["A VPN with TLS/IPsec", "WEP", "FTP", "Telnet"], a: 0, e: "A VPN encrypts the tunnel (IPsec or TLS). WEP is broken; FTP/Telnet are cleartext." },
  ...
  { cat: "Architecture & Cloud", q: "Immutable infrastructure and treating servers as disposable best supports which idea?", o: ["Replace rather than patch in place (cattle, not pets)", "Manual configuration drift", "Long-lived snowflake servers", "Shared admin passwords"], a: 0, e: "Immutable infra redeploys clean images instead of patching live hosts, reducing drift and persistence of compromise." },
  ...
  { cat: "PKI & Certificates", q: "What does an organization send to a CA to request a certificate?", o: ["A Certificate Signing Request (CSR)", "A CRL", "Its private key", "An OCSP response"], a: 0, e: "A CSR contains the public key and identifying info. You never send your private key to the CA." },
  ...
  { cat: "PKI & Certificates", q: "A certificate that secures example.com AND all of its subdomains is a:", o: ["Wildcard certificate (*.example.com)", "SAN certificate", "Self-signed certificate", "Code-signing certificate"], a: 0, e: "A wildcard (*.example.com) covers all subdomains. A SAN certificate covers multiple specific names/domains you list." },
  ...
  { cat: "Threat Actors", q: "Which threat actor has the highest resources and stealth, often state-funded?", o: ["Advanced Persistent Threat (nation-state)", "Script kiddie", "Hacktivist", "Insider"], a: 0, e: "Nation-state/APT actors have the most resources and patience, aiming for long-term espionage with low detection." },
  ...
  { cat: "Threat Actors", q: "An attacker using prebuilt tools without deep technical skill is a(n):", o: ["Unskilled attacker (script kiddie)", "Nation-state", "APT", "Insider threat"], a: 0, e: "Unskilled attackers rely on others' tools/scripts — low sophistication, but still dangerous with powerful tooling." },
  ...
  { cat: "Application Attacks", q: "Exploiting the gap between when a value is checked and when it is used is a:", o: ["Race condition (TOCTOU)", "Buffer overflow", "Replay attack", "XSS"], a: 0, e: "Time-of-check to time-of-use: state changes between validation and use. Fixed with locking/atomic operations." },
  ...
  { cat: "Application Attacks", q: "Coercing a server into making requests to internal resources on the attacker's behalf is:", o: ["Server-Side Request Forgery (SSRF)", "CSRF", "XSS", "Clickjacking"], a: 0, e: "SSRF abuses a server to reach internal systems (e.g., cloud metadata endpoints). Validate/allow-list outbound destinations." },
  { cat: "Application Attacks", q: "The single best defense against SQL injection is:", o: ["Parameterized queries / prepared statements", "Longer passwords", "Disabling cookies", "Using UDP"], a: 0, e: "Parameterized queries separate code from data so input can't alter the query. Add input validation and least-privilege DB accounts." },
  ...
  { cat: "Hardening & Endpoint", q: "Which tool alerts when critical system files change unexpectedly?", o: ["File Integrity Monitoring (FIM)", "DHCP", "NAT", "A load balancer"], a: 0, e: "FIM hashes/monitors important files and flags unauthorized changes — useful for spotting tampering or rootkits." },
  ...
  { cat: "Hardening & Endpoint", q: "In Windows domains, which tool centrally enforces security settings across many machines?", o: ["Group Policy (GPO)", "Syslog", "OCSP", "NAT"], a: 0, e: "Group Policy Objects push standardized configuration/hardening to domain-joined systems at scale." },
  ...
  { cat: "Vulnerability Management", q: "A penetration tester is given NO prior knowledge of the environment. This is a:", o: ["Black-box (unknown environment) test", "White-box test", "Gray-box test", "Compliance audit"], a: 0, e: "Black-box simulates an outsider with no info. White-box = full knowledge; gray-box = partial knowledge." },
  ...
  { cat: "Vulnerability Management", q: "Which exercise pits an attacking team against a defending team?", o: ["Red vs. Blue (with Purple-team collaboration)", "Tabletop walkthrough", "Failover test", "Gap analysis"], a: 0, e: "Red attacks, Blue defends; Purple-teaming has them collaborate to improve. A White team referees." },
  { cat: "Vulnerability Management", q: "Gathering intelligence from publicly available sources is:", o: ["OSINT (open-source intelligence)", "A credentialed scan", "Pivoting", "Privilege escalation"], a: 0, e: "OSINT collects data from public sources (websites, social media, DNS, leaks) — common in reconnaissance and threat intel." },
  ...
  { cat: "Digital Forensics", q: "Hashing a disk image before and after analysis proves which property?", o: ["Integrity (the evidence wasn't altered)", "Confidentiality", "Availability", "Non-repudiation of the attacker"], a: 0, e: "Matching hashes before/after show the evidence is unchanged, preserving integrity and court admissibility." },
  ...
  { cat: "Wireless & Mobile", q: "Which Wi-Fi feature is commonly disabled because its PIN is easily brute-forced?", o: ["WPS (Wi-Fi Protected Setup)", "WPA3-SAE", "802.1X", "PMF"], a: 0, e: "WPS's 8-digit PIN is vulnerable to brute force; best practice is to disable it." },
  ...
  { cat: "Wireless & Mobile", q: "Which lets an organization enforce policy, encryption, and remote wipe on phones?", o: ["MDM (Mobile Device Management)", "MAC filtering", "OCSP", "NAT"], a: 0, e: "MDM/UEM centrally manages mobile devices — policy, containerization (separating work/personal data), and remote wipe." },
  ...
  { cat: "Governance & Third-Party", q: "Which document defines the security requirements for connecting two organizations' systems?", o: ["ISA (Interconnection Security Agreement)", "NDA", "SLA", "BPA"], a: 0, e: "An ISA specifies the security controls for a system interconnection. An MOU captures broader intent; an SOW lists specific work." },
  ...
  { cat: "Governance & Third-Party", q: "Which document specifies exact deliverables, timeline, and tasks for a project?", o: ["SOW (Statement of Work)", "MOU", "NDA", "AUP"], a: 0, e: "A Statement of Work details specific deliverables/milestones, often under a Master Service Agreement (MSA) that sets overall terms." },
  ...
  { cat: "Governance & Third-Party", q: "An SBOM (Software Bill of Materials) primarily helps an organization:", o: ["Know which components/dependencies are in software to assess vulnerabilities", "Encrypt all disks", "Balance network load", "Replace passwords with biometrics"], a: 0, e: "An SBOM inventories components so you can quickly assess exposure when a dependency (e.g., Log4j) turns out vulnerable." },
  ...
  { cat: "Risk & Governance", q: "A metric that gives early warning of rising risk (e.g., a spike in failed logins) is a:", o: ["KRI (Key Risk Indicator)", "KPI", "SLE", "RTO"], a: 0, e: "KRIs flag increasing risk exposure. KPIs measure performance toward goals." },
  ...
  { cat: "IAM", q: "Which solution vaults, monitors, and time-limits administrator credentials?", o: ["Privileged Access Management (PAM)", "DLP", "WAF", "SIEM"], a: 0, e: "PAM vaults privileged credentials, brokers sessions, and grants just-in-time elevated access with full logging." },
  ...
  { cat: "IAM", q: "In biometrics, the point where the false acceptance and false rejection rates are equal is the:", o: ["Crossover Error Rate (CER)", "FAR", "FRR", "TPR"], a: 0, e: "CER (a.k.a. EER) is where FAR = FRR; a lower CER means a more accurate system. FAR is the dangerous error (wrong person accepted)." },
  { cat: "IAM", q: "A 6-digit code from an authenticator app that changes every 30 seconds is:", o: ["TOTP (time-based one-time password)", "HOTP", "A smart card", "A CAC"], a: 0, e: "TOTP derives a code from a shared secret + current time. HOTP is counter-based. Both are 'something you have'." },
  ...
  { cat: "Security Operations", q: "Automated, repeatable response steps executed by a SOAR platform are defined in:", o: ["Playbooks / runbooks", "CRLs", "CSRs", "SLAs"], a: 0, e: "Playbooks codify response workflows so SOAR can automate triage/containment, cutting response time and human error." },
  ...
  { cat: "Threats & Attacks", q: "An attacker maps their MAC to the gateway's IP to intercept LAN traffic. This is:", o: ["ARP poisoning (spoofing)", "DNS poisoning", "VLAN hopping", "Smurf attack"], a: 0, e: "ARP poisoning corrupts the ARP cache to become an on-path attacker. Dynamic ARP Inspection and static entries mitigate it." },
  ...
  { cat: "Network Security", q: "What does a Next-Generation Firewall (NGFW) add over a traditional firewall?", o: ["Application awareness plus deep packet inspection/IPS", "Only blocking ports", "DHCP leasing", "Disk encryption"], a: 0, e: "NGFWs inspect at the application layer, identify apps/users, and integrate IPS and threat intelligence." },
  ...
  { cat: "Network Security", q: "A hardened host used to administer devices inside a secure zone is a:", o: ["Jump server (jump box)", "Honeypot", "Load balancer", "Proxy"], a: 0, e: "A jump server is a controlled, monitored pivot point for admin access into a segmented/secure network." },

  // ----- Architecture & Cloud, additional (Domain 3) -----
  { cat: "Architecture & Cloud", q: "Managing infrastructure through version-controlled configuration files is:", o: ["Infrastructure as Code (IaC)", "Shadow IT", "Tokenization", "Air gapping"], a: 0, e: "IaC provisions infrastructure via code for consistency/repeatability — but misconfigurations propagate fast, so review templates." },
  { cat: "Architecture & Cloud", q: "Containers differ from virtual machines mainly because they:", o: ["Share the host OS kernel (lighter weight)", "Each include a full guest OS", "Cannot be orchestrated", "Require a separate hypervisor per app"], a: 0, e: "Containers share the host kernel (fast, dense) but provide less isolation than VMs, which run full guest OSes on a hypervisor." },
  ...
  { cat: "Physical Security", q: "A two-door entry letting only one person through at a time, to stop tailgating, is a(n):", o: ["Access control vestibule (mantrap)", "Turnstile bypass", "Bollard", "Faraday cage"], a: 0, e: "An access control vestibule traps a person between two interlocking doors, preventing tailgating/piggybacking." },
  ...
  { cat: "Physical Security", q: "Following an authorized employee through a secure door without badging is:", o: ["Tailgating (piggybacking)", "Shoulder surfing", "Dumpster diving", "Pretexting"], a: 0, e: "Tailgating bypasses access control by trailing someone in. Mantraps, turnstiles, and awareness training reduce it." },
  { cat: "Physical Security", q: "A sensor that detects body heat to trigger an alarm is which type?", o: ["Infrared (PIR) motion sensor", "Microwave jammer", "Faraday cage", "Proximity card"], a: 0, e: "Passive infrared detects heat/movement. Layering sensor types (infrared, microwave, ultrasonic) reduces false alarms and gaps." },
];
